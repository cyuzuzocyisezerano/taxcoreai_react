...
async function checkAndMigrate() {
  ...
  try {
    ...
    const taxpayerCount = await pool.query('SELECT COUNT(*) FROM taxpayers')
    const documentCount = await pool.query('SELECT COUNT(*) FROM documents')
    const userCount = await pool.query('SELECT COUNT(*) FROM users')

    console.log('Current PostgreSQL data:')
    console.log(`- Taxpayers: ${taxpayerCount.rows[0].count}`)
    console.log(`- Documents: ${documentCount.rows[0].count}`)
    console.log(`- Users: ${userCount.rows[0].count}`)

    if (Number(taxpayerCount.rows[0].count) === 0) {
      console.log('\n⚠️  Database is empty! Running migration...\n')
      ...
      try {
        ...
        if (db.documents && db.documents.length > 0) {
          for (const doc of db.documents) {
            ...
            const validStatuses = ['Active', 'Archived', 'Expired', 'Pending Review', 'Flagged']
        ...
        console.log('Then try searching again at /search-retrieval')
      ...
      console.log('If search still shows 0 results, try:')
